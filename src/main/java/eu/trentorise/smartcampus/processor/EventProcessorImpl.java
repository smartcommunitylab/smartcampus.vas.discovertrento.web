	...
	protected static Map<String, Object> populateBaseDTData(DomainObject dObj,
		...
		if (content.get("entityId") != null) {
			if (content.get("entityId") instanceof Long)
				eo.setEntityId((Long) content.get("entityId"));
			else
				eo.setEntityId(Long.parseLong(content.get("entityId")
						.toString()));
		}
		...
		if (communityData != null) {
			...
			if (ratings != null && !ratings.isEmpty()) {
				for (Map<String, Object> rating : ratings) {
					ratingList.add(new Rating((String) rating.get("user"),
							(Integer) rating.get("value")));
				}
			}
			...
			if (tagMapList != null) {
				for (Map<String, Object> tagMap : tagMapList) {
					...
					if (id instanceof Long)
						...
						c.setId(Long.parseLong(id.toString()));
